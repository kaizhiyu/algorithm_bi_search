...
public class XiaoNaClient implements Runnable {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 10; i++) {
            XiaoNaClient c = new XiaoNaClient();
            c.init();
            new Thread(c).start();
        }
    }
...
//                                channel.write(XiaoNaCharsetHelper.encode(CharBuffer.wrap(getWord())));
                                sleep();
                            } else {
                                key.cancel();
                            }
...
//                        System.out.println(Thread.currentThread().getId() + "---" + answer);
                        ...
                        String word = getWord();
                        if (word != null) {
//                            channel.write(XiaoNaCharsetHelper.encode(CharBuffer.wrap(word)));
                        } else {
                            isOver = true;
                        }
                        sleep();

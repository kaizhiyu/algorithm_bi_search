...
public class Testsss {
    ...
    // 取数xxx上的第d位数字
    static int digit(int num, int d){
        int pow = 1;
        while (--d > 0){
            pow *= 10;
        }
    ...
    // 基数排序
    static int[]  RadixSort(int[] input, int n){
        // 临时数组，用来存放排序过程中的数据
...
//        int[] test(arr, arr + sizeof(arr) / sizeof(arr[0]));
//        cout << "排序前:";
...
//        cout << endl;
        ...
        int[] newArr = RadixSort(result, result.length);
